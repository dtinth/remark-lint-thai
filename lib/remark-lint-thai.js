...
const rules = [
  ...
  {
    pattern:
      /([คฅฆงชซฌญฑฒณทธนพฟภมยฬฮ]|(?<![กจฎฏดตบปอ])[รลว])([ิีึืุูั]?)(๊|๋)/g,
    check(match) {
      return match[3] === '๊'
  ...
  spell(/(?<!แจ้งวัฒ|ฐา|มีมา|มุ่งมา|น้ำปา|อาส|ธรรมสว|พาช|หาย|ช|สถา)(นะค่ะ)/g, [
    'นะคะ',
    'น่ะค่ะ',
  ]),
  ...
  spell(/เวอ(?:ร์)?(?:ชัน|ชั่น)/g, 'เวอร์ชัน'),
  ...
  spell(/โ[พป]รเจ็?(?:กต์|ค)/g, 'โปรเจกต์'),
  spell(/อา(?:ร์)?เรย์/g, 'อาร์เรย์'),
  spell(/(?:อ๊|อ็|อ)อบเจ็?(?:กต์|ค|ก)/g, 'อ็อบเจกต์'),
  spell(/(ซอ(?:ร์)?[ซส])โค้ด/g, 'ซอร์ซโค้ด'),
  spell(/คอนเ(?:ท|ท้)น(?:ต์|ท์)/g, 'คอนเทนต์'),
  spell(/อั[ปพ]โหลด/g, 'อัปโหลด'),
  spell(/(?:อัป|อัพ|อัฟ)(?:เดท|เดต)/g, 'อัปเดต'),
  ...
  // https://th.m.wiktionary.org/wiki/%E0%B9%82%E0%B8%9B%E0%B8%A3%E0%B9%82%E0%B8%A1%E0%B8%95
  spell(/โปรโห?ม[ทตด]/g, 'โปรโมต'),
]
