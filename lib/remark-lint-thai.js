...
const rules = [
  ...
  {
    pattern: /([คฅฆงชซฌญฑฒณทธนพฟภมยรลวฬฮ])([ิีึืุูั]?)(๊|๋)/g,
    check(match) {
      return match[3] === '๊'
      ? 'Low-class consonants may not use mai tri (use mai tho instead)'
      : /[งญนมยรลวฬ]/.test(match[1])
      ? 'Low-class consonants may not use mai chattawa (use "ห' + match[1] + '" instead)'
      ...
      for (const pattern of [].concat(r.pattern)) {
        ...
        for (
          ...
          const point = {
            ...
            end: loc.toPosition(node.position.start.offset + match.index + match[0].length - 1),
          }
          if (r.check) {
